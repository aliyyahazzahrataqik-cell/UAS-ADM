  ...
  return (
    <section className="catalog-section" style={{ minHeight: "100vh" }}>
      <div style={{ textAlign: "center", marginBottom: "3rem" }}>
        <h1 className="section-title">Katalog <span className="text-gradient">Produk</span></h1>
        <p className="section-subtitle">Temukan berbagai koleksi bunga terbaik untuk momen spesial Anda</p>
      </div>

      <div className="catalog-filter">
        <Link href="/produk" className="catalog-filter__btn catalog-filter__btn--active">
          Semua Produk
        </Link>
        {categories.map((cat) => (
          <Link key={cat.id} href={`/kategori/${cat.slug}`} className="catalog-filter__btn">
            {cat.name}
          </Link>
        ))}
      </div>
      ...
      <div className="products-grid" style={{ maxWidth: "1200px", margin: "0 auto" }}>
        {products.length === 0 ? (
          <div className="empty-state" style={{ gridColumn: "1/-1" }}>
            ...
            <p className="empty-state__text">Belum ada produk yang tersedia saat ini.</p>
          </div>
        ) : (
